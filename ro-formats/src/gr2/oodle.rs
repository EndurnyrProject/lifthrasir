    ...
    fn decompress_block(
        ...
        if size_idx >= self.size_windows.len() {
            return Err(Gr2Error::DecompressionFailed(
                "oodle: invalid length symbol".into(),
            ));
        }
        self.backref_size =
            self.size_windows[size_idx].decode_symbol(decoder, |d| d.decode_commit(65)) as u32;

        if decoder.failed || self.backref_size > 64 {
            return Err(Gr2Error::DecompressionFailed(
                "oodle: invalid range stream".into(),
            ));
        }
...
pub(crate) fn decompress(
    ...
    if compressed.len() < 36 {
        return Err(Gr2Error::DecompressionFailed("oodle: short header".into()));
    }
    let params = [
    ...
    if steps[0] > steps[1] || steps[1] > out.len() {
        return Err(Gr2Error::DecompressionFailed(
            "oodle: invalid stop offsets".into(),
        ));
    }
    let mut pos = 0usize;
