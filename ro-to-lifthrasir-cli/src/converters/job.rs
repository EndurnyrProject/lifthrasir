...
pub fn run(vfs: &GrfVfs, out: &Path) -> anyhow::Result<()> {
    let mut sources = Vec::with_capacity(LUB_PATHS.len());
    for path in LUB_PATHS {
        let bytes = vfs
            .read(path)
            .with_context(|| format!("job lub not found in GRFs: {path}"))?;
        sources.push(decompile(&bytes).with_context(|| format!("decompiling {path}"))?);
    }

    let lua = lua::new_vm().map_err(lua_err)?;
    lua::exec_chunk(&lua, &sources[0]).map_err(lua_err)?;
    lua::install_job_metatables(&lua).map_err(lua_err)?;
    lua::exec_chunk(&lua, &sources[1]).map_err(lua_err)?;
    lua::exec_chunk(&lua, &sources[2]).map_err(lua_err)?;
    lua::exec_chunk(&lua, &sources[3]).map_err(lua_err)?;
