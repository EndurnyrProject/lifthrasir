use bevy::prelude::*;
use crate::components::RoAnimationController;
use crate::assets::{RoSpriteAsset, RoActAsset, convert_indexed_to_rgba, create_bevy_image, calculate_sprite_scale};
            ...
            if controller.timer >= controller.current_delay {
                ...
                if let Some(action_seq) = action.actions.get(controller.action_index) {
                    ...
                    if controller.animation_index >= action_seq.animations.len() {
                        ...
                        if controller.action_index >= action.actions.len().min(MAX_DISPLAYED_ACTIONS) {
                            controller.action_index = 0; // Loop back to first action
                        }
                        debug!("Switching to action {}", controller.action_index);
                    }
                ...
                if let Some(animation) = action_seq.animations.get(controller.animation_index) {
                    if let Some(first_layer) = animation.layers.first() {
                        ...
                        if let Some(sprite_frame) = sprite.frames.get(sprite_index) {
                            ...
                            // Calculate scale
                            let scale = calculate_sprite_scale(sprite_frame.width as u32, sprite_frame.height as u32);
                            ...
                            let new_transform = Transform::from_translation(_transform.translation + layer_offset)
