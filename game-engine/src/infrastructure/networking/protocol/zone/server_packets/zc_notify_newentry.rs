...
pub const ZC_NOTIFY_NEWENTRY: u16 = 0x09FE;

/// ZC_NOTIFY_NEWENTRY (0x09FE) - Zone Server → Client
///
/// Sent when an entity (player, NPC, monster, etc.) spawns or appears in the client's view.
...
///
/// This packet is identical to STANDENTRY except it DOES NOT include the `state` field.
///
/// # Packet Structure
...
#[derive(Debug, Clone)]
pub struct ZcNotifyNewentryPacket {
...
impl ServerPacket for ZcNotifyNewentryPacket {
    const PACKET_ID: u16 = ZC_NOTIFY_NEWENTRY;
    ...
    fn parse(data: &[u8]) -> io::Result<Self> {
        if data.len() < 4 {
            return Err(io::Error::new(
                ...
                format!(
                    "ZC_NOTIFY_NEWENTRY packet too short: expected at least 4 bytes, got {}",
                    data.len()
                ),
            ));
        }
        ...
        if data.len() < packet_length as usize {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "ZC_NOTIFY_NEWENTRY incomplete: expected {} bytes, got {}",
                    packet_length,
                    data.len()
                ),
            ));
