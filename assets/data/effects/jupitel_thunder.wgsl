// Jupitel Thunder detonation. One additive billboard drawing the whole electric
// burst: a flickering white-hot core, jagged lightning bolts radiating from the
// center (re-randomized crackle_hz times per second so they visibly arc), and a
// thin expanding shock ring. The `factor` uniform (0->1 over the burst, driven
// by the ECS FactorRamp) shapes the envelopes: instant bright pop, crackling
// simmer, then everything fades. Crackle jitter comes from the view globals
// clock. Unlit, additive blend, camera-facing built in the vertex stage.
#import bevy_pbr::mesh_functions::get_world_from_local
#import bevy_pbr::mesh_view_bindings::{view, globals}

const TAU: f32 = 6.28318530718;

struct JupitelParams {
    primary_color: vec4<f32>,
    secondary_color: vec4<f32>,
    shape: vec4<f32>,   // x=emission y=bolt_count z=crackle_hz
    factor: f32,
}

@group(#{MATERIAL_BIND_GROUP}) @binding(0) var<uniform> material: JupitelParams;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Shared re-seed: every bolt and the core flicker jump together, which is
    // exactly how a strobing electric arc reads.
    let reseed = floor(globals.time * crackle_hz);

    let appear = smoothstep(0.0, 0.04, f);
    let fade = 1.0 - smoothstep(0.65, 1.0, f);

    // White-hot flickering core with a soft blue halo: pops at full brightness,
    // settles into a simmer, then dies with the fade.
    ...
    // Jagged radial bolts. Each bolt leaves the center at a random angle and
    // wanders sideways with radius via value noise; `abs(d) * r` is the
    // world-space distance to the bolt path, so bolts keep constant thickness
    // instead of thinning into wedges. A wide dim pass under the sharp pass
    // gives each bolt a blue corona.
    var sharp = 0.0;
    var glow = 0.0;
    ...
    // Thin shock ring expanding to the rim over the first half of the burst,
    // angularly modulated so it reads as a pressure wave, not a compass circle.
    ...
    let color = (material.primary_color.rgb * hot + material.secondary_color.rgb * cool) * emission;
