...
use super::protocols::ro_char::{
    ...
    CharServerResponse, CharacterInfo, HC_ACCEPT_DELETECHAR, HC_ACCEPT_ENTER, HC_ACCEPT_MAKECHAR,
    HC_BLOCK_CHARACTER, HC_CHARACTER_LIST, HC_NOTIFY_ZONESVR, HC_PING, HC_REFUSE_DELETECHAR,
    HC_REFUSE_MAKECHAR, HC_SECOND_PASSWD_LOGIN, HcAcceptEnterPacket, HcBlockCharacterPacket,
    HcCharacterListPacket, HcNotifyZonesvrPacket, HcSecondPasswdLoginPacket,
};
        ...
        if let Some(ref mut stream) = self.connection {
            ...
            loop {
                match stream.read(&mut buffer) {
                    ...
                    Err(e) => {
                        return Err(NetworkError::ConnectionFailed(e));
                    }
                }
            }
        }
