...
fn titlebar() -> impl Scene {
    bsn! {
        ...
        on(on_titlebar_drag)
        Children [
            glyph_icon("bag", 16.0, theme::GOLD),
            (
                Text("Inventory")
                TextFont {
...
fn body_container() -> impl Scene {
    bsn! {
        ...
        Node {
            flex_direction: FlexDirection::Column,
            padding: {UiRect { left: Val::Px(14.0), right: Val::Px(14.0), top: Val::Px(10.0), bottom: Val::Px(14.0) }},
        }
        ignore_picking()
    }
}
...
fn on_close(_: On<Activate>, mut window: Query<&mut Visibility, With<InventoryWindowRoot>>) {
    if let Ok(mut visibility) = window.single_mut() {
        *visibility = Visibility::Hidden;
    }
}

/// Drag the single inventory window by its titlebar; mirrors `make_draggable` but
/// resolves the root from its marker instead of a captured entity, so the whole
/// window spawns as one scene with no imperative drag wiring. Only the titlebar
/// itself moves the window: `Pointer<Drag>` bubbles up from the close button, so a
/// drag targeting it is ignored.
fn on_titlebar_drag(
    drag: On<Pointer<Drag>>,
    titlebars: Query<(), With<InventoryTitlebar>>,
    mut roots: Query<&mut Node, With<InventoryWindowRoot>>,
) {
