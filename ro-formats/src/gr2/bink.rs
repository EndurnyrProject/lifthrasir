...
fn ac_band(dec: &mut Decoder, rd: &mut Reservoir, mut band: Band<'_>) -> Result<(), Gr2Error> {
    ...
    loop {
        if r1 == 0 {
            if r2 == 0 {
                budget = budget
                    .checked_sub(1)
                    .ok_or_else(|| Gr2Error::DecompressionFailed("bink: band overrun".into()))?;
        ...
        let level = m
            .levels
            .get_mut(cls)
            .ok_or_else(|| Gr2Error::DecompressionFailed("bink: invalid level class".into()))?;
        let mut lvl = level.decode_symbol(dec, |d| d.decode_commit(esc_total)) as i32;
        if lvl != 0 {
            if rd.pull(1) != 0 {
                lvl = -lvl;
            }
...
fn decode_plane(
    ...
    if dec.failed {
        return Err(Gr2Error::DecompressionFailed(
            "bink: invalid range stream".into(),
        ));
    }
    Ok(end)
}
...
pub(crate) fn decode_planes(
    ...
    for _ in 0..n {
        ...
        let source = pixels.get(off..).ok_or(Gr2Error::UnexpectedEof)?;
        let consumed = decode_plane(source, &mut plane, w, h, &mut flags)?;
        off += consumed;
        planes.push(plane);
...
pub fn decode(
    ...
    height: i32,
    has_alpha: bool,
) -> Result<Vec<u8>, Gr2Error> {
    if !(1..=4096).contains(&width) || !(1..=4096).contains(&height) {
        return Err(Gr2Error::DecompressionFailed(
            "bink: invalid texture dimensions".into(),
        ));
    }
