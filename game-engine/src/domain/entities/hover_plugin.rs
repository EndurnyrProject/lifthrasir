...
use super::name_cache::EntityNameCache;
use super::name_request_system::{
    cache_cleanup_system, name_request_system, name_response_handler_system, CacheCleanupTimer,
};
use bevy::prelude::*;
...
/// - Resource initialization (EntityNameCache)
...
/// 5. `name_response_handler_system` - Caches received names
/// 6. `cache_cleanup_system` - Periodically cleans expired cache entries
///
/// # Integration
///
/// ```ignore
...
impl Plugin for EntityHoverPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<EntityNameCache>();
        app.init_resource::<HoverConfig>();
        app.insert_resource(CacheCleanupTimer(Timer::from_seconds(
            60.0,
            TimerMode::Repeating,
        )));
