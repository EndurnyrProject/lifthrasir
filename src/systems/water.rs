...
use crate::{
    assets::converters::decode_image_from_bytes,
    assets::loaders::{GrfAsset, RoGroundAsset, RoWorldAsset},
    components::{
        GrfMapLoader, MapLoader, WaterAnimation, WaterExtension, WaterMaterial, WaterSurface,
        map::MapData,
    },
...
pub fn load_water_system(
    ...
    for (entity, map_loader, grf_loader, map_data) in query.iter() {
        ...
        let water_extension = WaterExtension {
            water_data: crate::components::water::WaterData {
...
fn load_water_texture(
    ...
    // Generate water texture path using Korean "워터" (water) in the path
    // The GRF uses Korean directory names with proper encoding
    let texture_path = format!("data\\texture\\워터\\water{}{:02}.jpg", water_type, frame);

    info!("Attempting to load water texture: {}", texture_path);

    // Try to load the texture from GRF
    if let Some(texture_data) = grf_asset.grf.get_file(&texture_path) {
        match decode_image_from_bytes(&texture_data, &texture_path) {
            Ok(image) => {
                info!(
                    "Successfully loaded water texture: {} (size: {}x{})",
                    texture_path,
        ...
        let alt_paths = vec![
            format!("data/texture/워터/water{}{:02}.jpg", water_type, frame),
            format!("data\\texture\\water\\water{}{:02}.jpg", water_type, frame),
            format!("data/texture/water/water{}{:02}.jpg", water_type, frame),
        ];

        for alt_path in alt_paths {
            if let Some(texture_data) = grf_asset.grf.get_file(&alt_path) {
                match decode_image_from_bytes(&texture_data, &alt_path) {
                    Ok(image) => {
                        info!(
                            "Successfully loaded water texture from alternative path: {}",
                            alt_path
                        );
                        return Some(images.add(image));
                    }
                    Err(e) => {
                        warn!("Failed to decode water texture {}: {}", alt_path, e);
                    }
                }
            }
        }
    ...
    // If no texture is found, return None (will use default material color)
    None
}
